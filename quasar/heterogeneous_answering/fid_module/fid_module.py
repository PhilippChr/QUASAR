...
import quasar.heterogeneous_answering.fid_module.fid_utils as fid_utils
...
class FiDModule(HeterogeneousAnswering):
    def __init__(self, config):
        ...
        self.config = config
        self.path_to_fid = "quasar/heterogeneous_answering/fid_module/FiD"
    ...
    def _initialize_conda_dir(self):
        ...
        conda_dir = "/home/pchristm/anaconda3/envs/"
        if not conda_dir:
            raise Exception(
        ...
        # in case some environment is activated (which should be `quasar`), move one dir upwards
...
def main():
    if len(sys.argv) < 2:
        raise Exception(
            "python quasar/heterogeneous_answering/fid_module/fid_module.py --<FUNCTION> <PATH_TO_CONFIG> [<SOURCES_STR>]"
        )
