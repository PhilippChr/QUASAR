...
import quasar.heterogeneous_answering.seq2seq_answering.dataset_seq2seq_answering as dataset
from quasar.heterogeneous_answering.heterogeneous_answering import HeterogeneousAnswering
from quasar.heterogeneous_answering.seq2seq_answering.seq2seq_answering_model import (
...
class Seq2SeqAnsweringModule(HeterogeneousAnswering):
    ...
    def train(self, sources=("kb", "text", "table", "info"), **kwargs):
        ...
        train_path = os.path.join(data_dir, qu, ers, sources_str, "train_ers-convinse.jsonl")
        dev_path = os.path.join(data_dir, qu, ers, sources_str, "dev_ers-convinse.jsonl")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        raise Exception(
            "Usage: python quasar/heterogeneous_answering/seq2seq_answering/seq2seq_answering_module.py --<FUNCTION> <PATH_TO_CONFIG> [<SOURCES_STR>]"
        )
