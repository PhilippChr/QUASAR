...
class DatasetGNN(Dataset):
    def __init__(self, config, data_path=None, train=False, lazy_load=False):
        self.config = config
        ...
        # load data
        if lazy_load:
            self.len = len([f for f in os.listdir(data_path) if f.endswith(".pt")])
        else:
            self.instances = DatasetGNN.prepare_data(config, data_path, train)

    def __getitem__(self, idx):
        if getattr(self, "lazy_load", False):
            return torch.load(f"{self.data_path}/instance_{idx+1}.pt", weights_only=False)
        else:
    ...
    @staticmethod
    def prepare_lazy_load(config, data_path, lazy_load_path, train=False):
        """
        Create .jsonl for the dataset, which can be lazy-loaded.
        """
        # load data
        ans_pres_list = list()
        Path(lazy_load_path).mkdir(parents=True, exist_ok=True)
        with open(data_path, "rb") as fp_in:
            line = fp_in.readline()
            idx = 0
            while line:
                idx += 1
                ...
                for instance in instances:
                    torch.save(instance, f"{lazy_load_path}/instance_{idx}.pt")
                    ans_pres_list.append(1 if sum(instance["evidence_labels"]) > 0 else 0)
                line = fp_in.readline()
                if config.get("dev") and idx > 10:
                    break
    ...
    def prepare_turn(config, turn, train=False):
        ...
        if len(evidences) > max_evidences:
            evidences = evidences[:max_evidences] # assumes ordering among evidences
