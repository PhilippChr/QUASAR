...
from quasar.heterogeneous_answering.graph_neural_network.answering.answering import Answering
...
class MultitaskBilinearAnswering(Answering):
    ...
    def forward(self, batch, train, entity_mat, sr_vec, ev_mat, iteration=None, *args):
        ...
        answer_logits_before_mask = F.softmax(answer_outputs, dim=1)  # batch_size x num_ent x 1
        entity_mask = batch["entity_mask"]
        answer_logits = answer_logits_before_mask.squeeze() * entity_mask
        ...
        ev_logits_before_mask = F.softmax(ev_outputs, dim=1)  # batch_size x num_ev x 1
        evidence_mask = batch["evidence_mask"]
        ev_logits = ev_logits_before_mask.squeeze() * evidence_mask
        ...
        if "entity_labels" in batch and "evidence_labels" in batch:
            ...
            if self.config.get("gnn_loss_before_mask", False):
                answer_loss = self.loss_fct(
                    answer_logits_before_mask.view(-1), batch["entity_labels"].view(-1).float()
                )
            else:
                answer_loss = self.loss_fct(
            ...
            if self.config.get("gnn_loss_before_mask", False):
                evidence_loss = self.loss_fct(
                    ev_logits_before_mask.view(-1), batch["evidence_labels"].view(-1).float()
                )
            else:
                evidence_loss = self.loss_fct(
        ...
        if not train:
            ...
            if "gnn_max_output_evidences" in self.config:
                evidence_predictions = self.add_top_evidences(
                    batch, ev_logits.unsqueeze(dim=2), answer_logits.unsqueeze(dim=2), answer_predictions
                )
