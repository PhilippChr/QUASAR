...
from quasar.heterogeneous_answering.graph_neural_network.gnn_factory import GNNFactory
...
class GNNModule(HeterogeneousAnswering):
    ...
    def train(self, sources=("kb", "text", "table", "info"), train_path=None, dev_path=None):
        ...
        for epoch in range(epochs):
            ...
            train_loss = 0.0
            self.gnn.train()
            for _, instances in enumerate(tqdm(train_loader)):
            ...
            self.gnn.eval()
            with torch.no_grad():
                for _, instances in enumerate(tqdm(dev_loader)):
                    # move data to gpu (if possible)
                    GNNModule._move_to_cuda(instances)
                    # make prediction / forward pass
    ...
    def inference_on_turns(self, turns, sources=("kb", "text", "table", "info"), train=False, use_tqdm=True):
        ...
        with torch.no_grad():
            ...
            while start_index < len(instances):
                ...
                self.logger.info(f"GNN took {time.time() - start} seconds")
                ...
                for i, _ in enumerate(batch_instances):
                    ...
                    if "instance" in turn:
                        del turn["instance"]
                    if "top30_evidences" in turn:
                        del turn["top30_evidences"]
