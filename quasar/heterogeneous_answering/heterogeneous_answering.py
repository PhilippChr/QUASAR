...
from quasar.library.utils import store_json_with_mkdir, store_jsonl_with_mkdir, get_logger
...
class HeterogeneousAnswering:
    ...
    def inference_on_data_split(self, input_path, output_path, sources, train=False, jsonl=False):
        ...
        if input_path.endswith("jsonl"):
            ...
            with open(input_path, "r") as fp:
                data = json.load(fp)
            input_turns = [turn for conv in data for turn in conv["questions"]]
    ...
    def inference_on_turns(self, input_turns, sources=("kb", "text", "table", "info"), train=False, use_tqdm=True):
        ...
        GNN inference on the train set is enabled.
        """
        for turn in tqdm(input_turns, disable=not use_tqdm):
    ...
    def log_results(self, turns):
        ...
        self.logger.info(res_str)
        res_str = f"Gold answers - Hit@5 ({num_questions}): {hit_at_5}"
        self.logger.info(res_str)
