...
from quasar.evaluation import answer_presence
from quasar.evidence_retrieval_scoring.bm25_es import BM25Scoring
from quasar.evidence_retrieval_scoring.clocq_er import ClocqRetriever
from quasar.evidence_retrieval_scoring.evidence_retrieval_scoring import EvidenceRetrievalScoring
from quasar.library.utils import get_config
...
class RERS(EvidenceRetrievalScoring):
    ...
    def inference_on_turn(self, turn, sources=("kb", "text", "table", "info"), train=False):
        ...
        # answer presence
        if "answers" in turn:
            hit, answering_evidences = answer_presence(top_evidences, turn["answers"], relaxed=True)
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise Exception(
            "Usage: python quasar/evidence_retrieval_scoring/rers.py --<FUNCTION> <PATH_TO_CONFIG>"
        )
