...
# ers
from quasar.evidence_retrieval_scoring.clocq_bm25 import ClocqBM25
from quasar.evidence_retrieval_scoring.rers import RERS

# ha
from quasar.heterogeneous_answering.fid_module.fid_module import FiDModule
...
from quasar.heterogeneous_answering.seq2seq_answering.seq2seq_answering_module import (
    Seq2SeqAnsweringModule,
)
from quasar.heterogeneous_answering.quasar_answering import (
    QuasarAnsweringModule,
)
from quasar.library.utils import (
    get_config,
    get_logger,
    get_result_logger,
...
# qu
from quasar.question_understanding.naive_concat.naive_concat import NaiveConcat
from quasar.question_understanding.question_resolution.question_resolution_module import (
    QuestionResolutionModule,
)
from quasar.question_understanding.question_rewriting.question_rewriting_module import (
    QuestionRewritingModule,
)
from quasar.question_understanding.structured_representation.structured_representation_module import (
...
class Pipeline:
    def __init__(self, config, use_gold_answers, train=False):
        ...
        self.ha = self._load_ha(train=train)
    ...
    def _load_ha(self, train=False):
        """Instantiate HA stage."""
        ha = self.config["ha"]
        ...
        if ha == "fid":
            ...
            return LLaMaAnsweringModule(self.config, train=train)
        elif ha == "quasar":
            return QuasarAnsweringModule(self.config, train=train)
        else:
            raise ValueError(
...
def main():
    ...
    if len(sys.argv) < 3:
        raise Exception(
            "Usage: python quasar/pipeline.py <FUNCTION> <PATH_TO_CONFIG> [<SOURCES_STRING>]"
        )
    ...
    if function.startswith("--train"):
        ...
        pipeline = Pipeline(config, use_gold_answers=True, train=True)
